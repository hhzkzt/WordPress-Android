...
function build_app_bundle {
  ...
  bundletool build-apks --bundle=$BUILDDIR/$name \
                        --output=$BUILDDIR/$name.apks \
                        ...
                        --overwrite | tee -a $LOGFILE
  echo "APKs ready: $name.apks. Install with 'bundletool install-apks --apks=$BUILDDIR/$name.apks'" | tee -a $LOGFILE
